<template>
    ...
    <data-list :datas="heroDatas" >
      ...
      <data-coloum label="配合指数" prop="antiRate" width="20%"></data-coloum>
...
export default {
    ...
    methods:{
        ...
        fetchDatas(skill,ladder){
            // if(this.singelHero._id) return 
            console.log(skill,ladder)
            this.skill = skill || 'all'
            this.ladder = ladder|| 'all'
           ...
           .then(async res=>{
               ...
               let hero = res.data.OpponentsAndTeammates.team[this.skill][this.ladder].map(item=>{
